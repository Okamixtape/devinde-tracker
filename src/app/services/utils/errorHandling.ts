...
export function isErrorOfCode(
  ...
  if (error instanceof AppError) {
    if (typeof code === 'number') {
      ...
      return error.code === ErrorCodes[code]?.code;
    }
  }
  return false;
}
...
function logErrorToConsole(error: AppError): void {
  console.error(
    `[${error.timestamp}] [${error.severity}] [${error.category}] ${error.code}: ${error.message}`,
    error.details || '',
    error.stack || ''
  );
}
...
// Utility to create authentication errors
...
// Utility to wrap synchronous operations with error handling
export function tryCatch<T>(
  fn: () => T
): [T | null, ErrorResponse | null] {
  try {
    const result = fn();
    return [result, null];
  } catch (error) {
